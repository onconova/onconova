...
from measurement.measures import Volume as VolumeBase
...
def get_measurement(
    value: Any,
    measure: type[Measure] | type[BidimensionalMeasure] | None = None,
    unit: str | None = None,
    original_unit=None,
):
    """
    ...
    if not measure and (unit or original_unit):
        ...
        measure = next(
            (
                ...
                if (unit or original_unit) in measure.get_units()
            )
        )
    assert (
        measure is not None
    ), "Either measure, original_unit, or unit must be provided."
