...
class TumorBoardReviewProfile(OnconovaFhirBaseSchema, fhir.OnconovaTumorBoardReview):
    ...
    def discriminator(cls, concept: fhir.CodeableConcept) -> fhir.CodeableConcept:
        if (
            ...
            or concept.fhirpath_single("extension('http://onconova.github.io/fhir/StructureDefinition/onconova-ext-tumor-board-specialization').exists()")
        ):
            raise ValueError(f'The code {concept.coding[0].system}#{concept.coding[0].code} is not a valid unspecified tumor board code discriminator')
        return concept
    ...
    def onconova_to_fhir(
        ...
        if obj.category != TumorBoardSpecialties.UNSPECIFIED:
            raise ValueError('Only unspecified tumor boards are supported by this profile')
