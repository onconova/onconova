...
class CancerFamilyMemberHistoryProfile(OnconovaFhirBaseSchema, fhir.OnconovaCancerFamilyMemberHistory):
    ...
    def fhir_to_onconova(
        ...
        condition: fhir.OnconovaCancerFamilyMemberHistoryCancerCondition | None = obj.fhirpath_single("FamilyMemberHistory.condition.where(code.coding.code='363346000')")
        return schemas.FamilyHistoryCreate(
            ...
            relationship=CodedConcept.model_validate(obj.fhirpath_single("FamilyMemberHistory.relationship.coding")),
            hadCancer=condition is not None,
            onsetAge=condition.fhirpath_single("onsetAge.value") if condition else None,
            contributedToDeath=condition.fhirpath_single("extension('http://onconova.github.io/fhir/StructureDefinition/onconova-ext-family-member-history-condition-contributed-to-death').valueBoolean") if condition else None,
            morphology=CodedConcept.model_validate(coding) if condition and (coding:=condition.fhirpath_single("extension('http://onconova.github.io/fhir/StructureDefinition/onconova-ext-family-history-member-cancer-morphology').valueCodeableConcept.coding")) else None,
            topography=CodedConcept.model_validate(coding) if condition and (coding:=condition.fhirpath_single("extension('http://onconova.github.io/fhir/StructureDefinition/onconova-ext-family-history-member-cancer-topography').valueCodeableConcept.coding")) else None,            
        )
    ...
    def onconova_to_fhir(
        ...
        if obj.hadCancer:
            condition = fhir.OnconovaCancerFamilyMemberHistoryCancerCondition(
                    onsetAge=fhir.Age(value=obj.onsetAge, system="http://unitsofmeasure.org", code="a") if obj.onsetAge is not None else None
            )
            condition.extension = []
            if obj.contributedToDeath is not None:
                condition.extension.append(
                    fhir.FamilyMemberHistoryConditionContributedToDeath(
            ...
            if obj.topography:
                condition.extension.append(
                    fhir.FamilyMemberHistoryCancerTopography(
                        valueCodeableConcept=construct_fhir_codeable_concept(obj.topography)
                    )
                )
            if obj.morphology:
                condition.extension.append(
