...
class ImagingDiseaseStatusProfile(OnconovaFhirBaseSchema, fhir.OnconovaImagingDiseaseStatus):
    ...
    def fhir_to_onconova(
        ...
        return schemas.TreatmentResponseCreate(
            ...
            recist=CodedConcept.model_validate(obj.fhirpath_single("Observation.valueCodeableConcept.coding")),
            recistInterpreted=obj.fhirpath_single("Observation.valueCodeableConcept.extension('http://onconova.github.io/fhir/StructureDefinition/onconova-ext-treatment-response-recist-is-interpreted').valueBoolean"),
            assessedEntitiesIds=obj.fhirpath_values("Observation.focus.reference.replace('Condition/', '')"),
            assessedBodysites=[
                CodedConcept.model_validate(bodysite) for bodysite in bodysites 
            ] if (bodysites := obj.fhirpath_values("Observation.bodySite.coding")) else None
        )
    ...
    def onconova_to_fhir(
        ...
        resource.bodySite = construct_fhir_codeable_concept(obj.assessedBodysites[0]) if obj.assessedBodysites else None
        resource.focus = [
            Reference(
                reference=f"Condition/{conditionId}",
            ) for conditionId in obj.assessedEntitiesIds
        ] if obj.assessedEntitiesIds else None
