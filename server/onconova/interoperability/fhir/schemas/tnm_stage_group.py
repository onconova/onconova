...
from onconova.interoperability.fhir.models.TNMPrimaryTumorCategory import OnconovaTNMPrimaryTumorCategory
from onconova.interoperability.fhir.models.TNMRegionalNodesCategory import OnconovaTNMRegionalNodesCategory
from onconova.interoperability.fhir.models.TNMDistantMetastasesCategory import OnconovaTNMDistantMetastasesCategory
from onconova.interoperability.fhir.models.TNMLymphaticInvasionCategory import OnconovaTNMLymphaticInvasionCategory
from onconova.interoperability.fhir.models.TNMResidualTumorCategory import OnconovaTNMResidualTumorCategory
from onconova.interoperability.fhir.models.TNMSerumTumorMarkerLevelCategory import OnconovaTNMSerumTumorMarkerLevelCategory
from onconova.interoperability.fhir.models.TNMGradeCategory import OnconovaTNMGradeCategory
from onconova.interoperability.fhir.models.TNMVenousInvasionCategory import OnconovaTNMVenousInvasionCategory
from onconova.interoperability.fhir.models.TNMPerineuralInvasionCategory import OnconovaTNMPerineuralInvasionCategory
...
class TNMStageGroupProfile(OnconovaFhirBaseSchema, fhir.OnconovaTNMStageGroup):
    ...
    def discriminator(cls, concept: fhir.CodeableConcept) -> fhir.CodeableConcept:
        ...
        if not concept.coding or not (coding:=concept.coding[0]) or coding.code not in allowed_codes:
            raise ValueError(f'The code {coding.system}#{coding.code} is not a valid staging code discriminator')
        return concept
    ...
    def fhir_to_onconova(
        ...
        instance = schemas.TNMStagingCreate(
            ...
            caseId=obj.fhirpath_single("Observation.subject.reference.replace('Patient/', '')"),
        ...
        for member in obj.hasMember or []:
            if not member.reference or not '#' in member.reference:
                raise ValueError('The Observation.hasMember.reference element must be provided and be an internal reference, i.e. "#{internal_id}"')
            internal_id = member.reference.lstrip('#')            
            contained_resource = obj.fhirpath_single(f"Observation.contained.where(id='{internal_id}' and resourceType='Observation')")
            if not contained_resource:
                raise RuntimeError(f"Could not resolve referenced member with ID '{internal_id}' within the contained resources.")
            value = CodedConcept.model_validate(contained_resource.fhirpath_single('Observation.valueCodeableConcept.coding'))
    ...
    def onconova_to_fhir(
        cls, obj: schemas.TNMStaging
    ) -> fhir.OnconovaTNMStageGroup:
        ...
        resource.code = construct_fhir_codeable_concept(
            Coding(code="399588009", system="http://snomed.info/sct", display="Pathologic TNM stage grouping")
            if obj.pathological else
            Coding(code="399537006", system="http://snomed.info/sct", display="Clinical TNM stage grouping")
        )
        if obj.primaryTumor:
    ...
    @classmethod
    def add_fhir_tnm_category(cls, resource: fhir.OnconovaTNMStageGroup, value: CodedConcept, profile):
        ...
        contained_resource.code = construct_fhir_codeable_concept(cls.map_to_fhir("TNMCategories", profile))
