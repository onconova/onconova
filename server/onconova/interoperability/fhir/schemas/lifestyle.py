...
class LifestyleProfile(OnconovaFhirBaseSchema, fhir.OnconovaLifestyle):
    ...
    def fhir_to_onconova(
        cls, obj: fhir.OnconovaLifestyle
    ) -> schemas.LifestyleCreate:
        return schemas.LifestyleCreate(
            ...
            smokingStatus=(
                ...
                if (coding:= obj.fhirpath_single("Observation.component.where(code.coding.code='72166-2').valueCodeableConcept.coding")) 
                else None
            ),
            smokingPackyears=obj.fhirpath_single("Observation.component.where(code.coding.code='8664-5').valueQuantity.value"),
            smokingQuited=(
                Measure(value=quantity.value, unit=quantity.code) 
                if (quantity:= obj.fhirpath_single("Observation.component.where(code.coding.code='107339-4').valueQuantity")) 
                else None
            ),
            alcoholConsumption=(
                CodedConcept.model_validate(coding) 
                if (coding:= obj.fhirpath_single("Observation.component.where(code.coding.code='1106630-7').valueCodeableConcept.coding")) 
                else None
            ),
            recreationalDrugs=[
                CodedConcept.model_validate(drug)
                for drug in drugs 
            ] if (drugs:= obj.fhirpath_values("Observation.component.where(code.coding.code='C84368').valueCodeableConcept.coding")) 
            else None,
            exposures=[
                CodedConcept.model_validate(exposure)
                for exposure in exposures 
            ] if (exposures:= obj.fhirpath_values("Observation.component.where(code.coding.code='C16552').valueCodeableConcept.coding")) 
            else None,
            nightSleep=(
                Measure(value=quantity.value, unit=quantity.code) 
                if (quantity:= obj.fhirpath_single("Observation.component.where(code.coding.code='93832-4').valueQuantity")) 
                else None
            ),
        )
    ...
    def onconova_to_fhir(
        cls, obj: schemas.Lifestyle
    ) -> fhir.OnconovaLifestyle:
        ...
        if obj.smokingStatus:
            resource.component.append(
                fhir.OnconovaLifestyleSmokingStatus(
                    valueCodeableConcept=construct_fhir_codeable_concept(obj.smokingStatus)
                )
            ) 
        ...
        if obj.alcoholConsumption:
            resource.component.append(
                fhir.OnconovaLifestyleAlcoholConsumption(
                    valueCodeableConcept=construct_fhir_codeable_concept(obj.alcoholConsumption)
                )
            )             
