...
class CancerStageProfile(OnconovaFhirBaseSchema, fhir.OnconovaCancerStage):
    ...
    def get_orm_model(cls, obj):
        try:
            domain = cls.map_to_internal("stagingDomain", obj.fhirpath_single("Observation.code.coding"))
    ...
    def fhir_to_onconova(
        ...
        create_schema = cls.__create_schemas__[cls.map_to_internal("stagingDomain", obj.fhirpath_single("Observation.code.coding"))]
        common = {
            "externalSource": None,
        ...
        if create_schema is schemas.BreslowDepthCreate:
            return create_schema(
                **common,
                depth=Measure(
                    value=obj.fhirpath_single("Observation.component.where(code.coding.code='92839-0').valueQuantity.value"),
                    unit="mm"
                ),
                isUlcered=obj.fhirpath_single("Observation.component.where(code.coding.code='105600-1').valueBoolean"),
            )
        elif create_schema is schemas.FIGOStagingCreate or create_schema is schemas.RaiStagingCreate:
            return create_schema(
                **common,
                stage=CodedConcept.model_validate(obj.fhirpath_single("Observation.valueCodeableConcept.coding")),
                methodology=CodedConcept.model_validate(method) if (method:=obj.fhirpath_single("Observation.method.coding")) else None,
            )
        else:
            return create_schema(
                **common,
        ...
        if obj.stagingDomain == StagingDomain.BRESLOW:
            resource.component = [
                ...
                fhir.OnconovaCancerStageComponent(
                    code = fhir.CodeableConcept(coding=[Coding(code="92839-0", system="http://loinc.org")]),
                    valueQuantity=Quantity(value=obj.depth.value, code='mm', system="http://unitsofmeasure.org")
                ),
                # TODO: replace by specialized component one models are regenerated
                fhir.OnconovaCancerStageComponent(
                    code = fhir.CodeableConcept(coding=[Coding(code="105600-1", system="http://loinc.org")]),
