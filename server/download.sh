...
if ! command -v wget &> /dev/null; then
  ...
  if command -v apt-get &> /dev/null; then
    ...
    echo "Error: Package manager not recognized. Please install wget manually."
    exit 1
  fi
fi
...
TERMINOLOGY=$1
DATA_DIR=./onconova/terminology/external-sources
...
process_loinc() {
	...
	echo "-----------------------------------"
  if [ ! -f $ONCONOVA_LOINC_ZIPFILE_PATH ]; then
      echo -e "ERROR FILE NOT FOUND:\nPlease download the LOINC_*.zip file from (requires a LOINC login):\n\n\thttps://loinc.org/download/loinc-complete/ \n\nand specify the location of the zip file with the ONCONOVA_LOINC_ZIPFILE_PATH variable.\n"
      exit 1
  fi  
  unzip $ONCONOVA_LOINC_ZIPFILE_PATH -d $TEMP_DIR 
...
process_snomedct() {
	...
	echo "-----------------------------------"
  if [ ! -f $ONCONOVA_SNOMED_ZIPFILE_PATH ]; then
  ...
  mkdir $TEMP_DIR
  unzip $ONCONOVA_SNOMED_ZIPFILE_PATH -d $DATA_DIR
  ...
  rm -r $TEMP_DIR
  rm -r $DATA_DIR/SnomedCT_*
  echo -e "\n✓ Processing complete. Saved data to multiple snomed*.tsv files.\n"
}
