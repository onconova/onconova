...
from pop.terminology.resolver import CanonicalUrlResolver
...
class TestCanonicalUrlResolver(unittest.TestCase):
    ...
    def test_resolve_with_valid_loinc_url(self):
        ...
        with patch.object(self.resolver, 'resolve_LOINC_endpoint', return_value=expected_url) as mock_resolve:
    ...
    def test_resolve_with_valid_hl7_url(self):
        ...
        with patch.object(self.resolver, 'resolve_HL7_endpoint', return_value=expected_url) as mock_resolve:
            result = self.resolver.resolve(canonical_url)
            self.assertEqual(result, expected_url)
            mock_resolve.assert_called_once_with(canonical_url)
    ...
    def test_resolve_with_valid_vsac_url(self):
        ...
        expected_url = 'https://cts.nlm.nih.gov/fhir/res/ValueSet/123/$expand?_format=json'
        with patch.object(self.resolver, 'resolve_VSAC_endpoint', return_value=expected_url) as mock_resolve:
            result = self.resolver.resolve(canonical_url)
            self.assertEqual(result, expected_url)
            mock_resolve.assert_called_once_with(canonical_url)
    ...
    def test_resolve_with_valid_cts_url(self):
        ...
        with patch.object(self.resolver, 'resolve_CTS_endpoint', return_value=expected_url) as mock_resolve:
            result = self.resolver.resolve(canonical_url)
            self.assertEqual(result, expected_url)
            mock_resolve.assert_called_once_with(canonical_url)

    def test_resolve_with_valid_simplifier_url(self):
        canonical_url = 'https://simplifier.net/pop/ValueSets/123'
        expected_url = 'https://simplifier.net/pop/ValueSets/123/$download?format=json'
        with patch.object(self.resolver, 'resolve_Simplifier_endpoint', return_value=expected_url) as mock_resolve:
            result = self.resolver.resolve(canonical_url)
            self.assertEqual(result, expected_url)
            mock_resolve.assert_called_once_with(canonical_url)
