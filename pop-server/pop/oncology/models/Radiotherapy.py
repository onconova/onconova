...
class Radiotherapy(BaseModel):
    ...
    sessions = models.PositiveIntegerField(
        ...
        help_text = _("The total number of radiotherapy sessions over the treatment period."),        
    )
    ...
    termination_reason = termfields.CodedConceptField(
        ...
        help_text = _("Explanation for the premature or planned termination of the radiotherapy"),
    ...
    def description(self):
        dosages = f"{' and '.join([dosage.description for dosage in self.dosages.all()])}"
        settings = f"{' and '.join([dosage.description for dosage in self.settings.all()])}" or 'Radiotherapy'
        return f'{self.intent.capitalize()} - {settings} {dosages}'
...
class RadiotherapyDosage(BaseModel):
    ...
    fractions = models.PositiveIntegerField(
        ...
        help_text = _("The total number of radiotherapy fractions delivered over the treatment period."), 
        null = True, blank = True,       
    )
    ...
    irradiated_volume_qualifier = termfields.CodedConceptField(
        ...
        help_text = _("General qualifier for the anatomical location of the irradiated volume"),
    ...
    @property
    def description(self):
        fractions_text = f' over {self.fractions} fractions' if self.fractions is not None else ''
