...
class ComorbiditiesAssessment(BaseModel):
    ...
    score = AnnotationProperty(
        ...
        annotation=Case(
            When(panel=ComorbiditiesPanel.CHARLSON, then=CharlsonPanelDetails.get_score_annotation()),
            When(panel=ComorbiditiesPanel.NCI, then=NciPanelDetails.get_score_annotation()),
            When(panel=ComorbiditiesPanel.ELIXHAUSER, then=ElixhauserPanelDetails.get_score_annotation()),
            default=None,
