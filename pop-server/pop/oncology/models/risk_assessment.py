...
def validate_risk_classification(assessment):
    if assessment.methodology.code == "C136962":  # Follicular Lymphoma International Prognostic Index (FLIPI)
    ...
    elif assessment.methodology.code == "C181086":  # D'Amico Prostate Cancer Risk Classification
        assert assessment.risk.code in (
            "C102403",  # Low risk
            "C102402",  # Intermediate risk
            "C102401",  # High risk
        )
    elif assessment.methodology.code == "C127872":  # European Treatment Outcome Study (EUTOS) Score
        assert assessment.risk.code in (
            "C102403",  # Low risk
            "C102401",  # High risk
        )
    ...
    elif assessment.methodology.code == "C155843":  # International Metastatic Renal Cell Carcinoma Database Consortium (IMDC) Criteria
        assert assessment.risk.code in (
            "C155844",  # IMDC Favorable risk
            "C155845",  # IMDC Intermediate Risk
            "C155846",  # IMDC Poor risk
        )
    elif assessment.methodology.code == "C161805":  # International Prognostic Index (IPI) Risk Group
        assert assessment.risk.code in (
            "C161809",  # High Risk
    ...
    elif assessment.methodology.code == "C177562":  # European LeukemiaNet Risk Classification
        assert assessment.risk.code in (
    ...
    elif assessment.methodology.code == "C181085":  # USCF Cancer of the Prostate Risk Assessment Score
        assert assessment.risk.code in (
            "C102403",  # Low risk
            "C102402",  # Intermediate risk
            "C102401",  # High risk
        )
    elif assessment.methodology.code == "C162781":  # Mantle Cell Lymphoma International Prognostic Index (MIPI)
        assert assessment.risk.code in (
            "C102403",  # Low risk
            "C102402",  # Intermediate risk
            "C102401",  # High risk
        )
    elif assessment.methodology.code == "C181084":  # NCCN Prostate Cancer Risk Stratification for Clinically Localized Disease
        assert assessment.risk.code in (
    ...
    elif assessment.methodology.code == "C177308":  # Non-Seminomatous Germ Cell Tumor IGCCC Risk Classification
        assert assessment.risk.code in (
            "C177310",  # Good
            "C177311",  # Intermediate
            "C177312",  # Poor
        )
    elif assessment.methodology.code == "C142346":  # International Society of Urological Pathology Gleason Grade Group
        assert assessment.risk.code in (
