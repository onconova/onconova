...
class BundleParserTest(TestCase):
    ...
    def test_get_or_create_user_creates_external_user(self):
        ...
        self.assertTrue(User.objects.filter(username=self.original_user.username).exists())
    ...
    def test_completed_data_categories_import(self):
        ...
        self.bundle.completedDataCategories = {
            "Diagnosis": schemas.PatientCaseDataCompletionStatusSchema(status=True, timestamp="2024-01-01T12:00:00Z", username="doctor1")
        }
        ...
        self.assertTrue(models.PatientCaseDataCompletion.objects.filter(case=imported_case, category="Diagnosis").exists())
