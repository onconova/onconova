...
from fhir.resources.R4B.valueset import ValueSet, ValueSetCompose, ValueSetComposeInclude, ValueSetExpansion, \
                    ValueSetExpansionContains, ValueSetComposeIncludeConcept, ValueSetComposeIncludeFilter
from pop.terminology.services import download_canonical_url, expand_valueset, CodedConcept, download_valueset, download_codesystem, \
...
class TestExpandValueSet(unittest.TestCase):
    ...
    def test_expansion_with_predefined_expansion(self):
        ...
        valuesetdef = ValueSet.construct(**dict(
            expansion=ValueSetExpansion.construct(**dict(
                contains=[
                    ValueSetExpansionContains(code='code1', system='system1', version='version1'),
                    ValueSetExpansionContains(code='code2', system='system2', version='version2')
                ]
            ))
        ))
        expected_concepts = [
    ...
    def test_expansion_with_composition_rules_include(self):
        ...
        with patch('pop.terminology.services.follow_valueset_composition_rule') as mock_follow_rule:
...
class TestFollowValueSetCompositionRule(unittest.TestCase):
    def test_rule_with_system_no_concept_no_filter(self):
        ...
        with patch("pop.terminology.services.download_codesystem", return_value=codesystem):
            result = follow_valueset_composition_rule(rule)
            self.assertEqual(result, [CodedConcept(code="code1", system="http://example.com/codesystem")])

    def test_rule_with_system_and_concept(self):
        rule = ValueSetComposeInclude(system="http://example.com/codesystem", concept=[ValueSetComposeIncludeConcept(code="code1")])
        codesystem = {
             'code1': CodedConcept(code="code1", system="http://example.com/codesystem")
        }
        with patch("pop.terminology.services.download_codesystem", return_value=codesystem):
    ...
    def test_rule_with_system_and_filter(self):
        rule = ValueSetComposeInclude(system="http://example.com/codesystem", filter=[ValueSetComposeIncludeFilter.construct(op=FilterOperator.IS_A, value="code1")])
        codesystem = {
             'code1': CodedConcept(code="code1", system="http://example.com/codesystem")
        }
        with patch("pop.terminology.services.download_codesystem", return_value=codesystem):
    ...
    def test_rule_with_value_set(self):
        ...
        valueset_concepts = [CodedConcept(code="code1", system="http://example.com/valueset")]
        with patch("pop.terminology.services.download_valueset", return_value=valueset_concepts):
            result = follow_valueset_composition_rule(rule)
            self.assertEqual(result, [CodedConcept(code="code1", system="http://example.com/valueset")])

    def test_rule_with_system_and_value_set(self):
        rule = ValueSetComposeInclude(system="http://example.com/codesystem", valueSet=["http://example.com/valueset"])
        codesystem = {
             'code1': CodedConcept(code="code1", system="http://example.com/codesystem")
        }
        valueset = MagicMock()
        valueset.concepts.return_value = [CodedConcept(code="code1", system="http://example.com/valueset")]
        with patch("pop.terminology.services.download_codesystem", return_value=codesystem):
            with patch("pop.terminology.services.download_valueset", return_value=valueset):
                result = follow_valueset_composition_rule(rule)
                self.assertEqual(result, [CodedConcept(code="code1", system="http://example.com/codesystem")])

    def test_rule_with_multiple_value_sets(self):
        rule = ValueSetComposeInclude(valueSet=["http://example.com/valueset1", "http://example.com/valueset2"])
        valueset1_concepts = [
        ...
        with patch("pop.terminology.services.download_valueset", side_effect=[valueset1_concepts, valueset2_concepts]):
            result = follow_valueset_composition_rule(rule)
            self.assertEqual(result, [CodedConcept(code="code2", system="http://example.com/codesystem")])
...
class TestCollectCodedConceptTerminology(unittest.TestCase):
    ...
    def test_force_reset(self):
        CodedConceptTestModel.objects.create(code='code1', system='system1', display='display1')
    ...
    def test_model_based_on_codesystem(self, mock_download_codesystem):
        ...
        collect_codedconcept_terminology(CodedConceptTestModel)
        mock_download_codesystem.assert_called_once_with(CodedConceptTestModel.codesystem)
        self.assertEqual(CodedConceptTestModel.objects.count(), 2)
        self.assertTrue(CodedConceptTestModel.objects.filter(code='code1').exists())
        self.assertTrue(CodedConceptTestModel.objects.filter(code='code2').exists())
    ...
    def test_updating_existing_concepts(self, mock_download_valueset):
        CodedConceptTestModel.valueset = 'https://example.com/valueset'
        CodedConceptTestModel.objects.create(code='code1', system='system1', display='wrong_display')
        CodedConceptTestModel.objects.create(code='code2', system='system1', display='display2')
        mock_download_valueset.return_value=[
            CodedConcept(code='code1', system='system1', display='display1'),
            CodedConcept(code='code2', system='system1', display='display2'),
        ]
        collect_codedconcept_terminology(CodedConceptTestModel)
        self.assertEqual(CodedConceptTestModel.objects.count(), 2)
        self.assertTrue(CodedConceptTestModel.objects.filter(display='display1').exists())
        self.assertTrue(CodedConceptTestModel.objects.filter(display='display2').exists())
    ...
    def test_updating_concept_relationships(self, mock_download_valueset):
        ...
        mock_download_valueset.return_value=[
            CodedConcept(code='code1', system='system1', display='display1'),
            CodedConcept(code='code2', system='system1', display='display2', parent='code1'),
        ]
        collect_codedconcept_terminology(CodedConceptTestModel)
        self.assertEqual(CodedConceptTestModel.objects.count(), 2)
    ...
    def test_prune_danling_concepts(self, mock_download_valueset):
        ...
        self.assertTrue(CodedConceptTestModel.objects.filter(display='display1').exists())
        self.assertTrue(not CodedConceptTestModel.objects.filter(display='display2').exists())
