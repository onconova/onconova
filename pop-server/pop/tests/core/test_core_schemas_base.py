...
from pop.core.schemas.factory.base import OrmMetadataMixin, ANONYMIZED_STRING
...
class TestSchemaAnonymization(unittest.TestCase):
    ...
    class TestGetSchema(ModelGetSchema):
        identifier: Optional[str] = Field(default=None)
        date: Optional[datetime] = Field(default=None)
        caseId: str = Field(default='test')
        config = SchemaConfig(model=UntrackedMockBaseModel, anonymization_fields=('date','identifier'), anonymization_key='caseId')

    def test_create_model_schema_has_not_is_anonymized_field(self):
        instance = self.TestCreateSchema(id=uuid4(), description='test')
        self.assertFalse(hasattr(instance, 'isAnonymized'))
        
    def test_get_model_schema_has_is_anonymized_field(self):
        instance = self.TestGetSchema(id=uuid4(), description='test')
        self.assertTrue(hasattr(instance, 'isAnonymized'))
        
    def test_get_model_schema_is_not_anonymized_by_default(self):
        instance = self.TestGetSchema(id=uuid4(), description='test')
        self.assertFalse(instance.isAnonymized)

    def test_date_is_properly_anonymized(self):
        original_date = datetime(2000,1,1)
        instance = self.TestGetSchema(id=uuid4(), description='test', date=original_date, isAnonymized=True)
        self.assertNotEqual(instance.date, original_date)

    def test_date_can_be_deanonymized(self):
        original_date = datetime(2000,1,1)
        instance = self.TestGetSchema(id=uuid4(), description='test', date=original_date, isAnonymized=False)
    ...
    def test_relative_dates_are_preserved_when_anonymized(self):
        ...
        instance1 = self.TestGetSchema(id=uuid4(), description='test', date=original_date1, isAnonymized=True)
        instance2 = self.TestGetSchema(id=uuid4(), description='test', date=original_date2, isAnonymized=True)
    ...
    def test_string_is_properly_anonymized(self):
        ...
        instance = self.TestGetSchema(id=uuid4(), description='test', identifier=original_identifier, isAnonymized=True)
    ...
    def test_string_can_be_deanonymized(self):
        original_identifier = '123456789'
        instance = self.TestGetSchema(id=uuid4(), description='test', identifier=original_identifier, isAnonymized=False)
