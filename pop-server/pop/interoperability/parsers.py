    ...
    def __init__(self, bundle: PatientCaseBundle):
        ...
        self.list_fields = [
            field_name for field_name, field_info in self.bundle.model_fields.items()
                if get_origin(field_info.annotation) is list and field_name not in ['updatedBy']
        ]
                ...
                for resource in getattr(self.bundle, list_field):
                    ...
                    for nested_resource_details in self.nested_resources.get(list_field,[]):
                        getattr(orm_resource, nested_resource_details.orm_related_name).set([
                            self.import_resource(nested_resource, instance=nested_resource_details.instance_init(orm_resource))
