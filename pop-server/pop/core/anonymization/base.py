...
def anonymize_clinically_relevant_date(
    ...
    timeshift = hash_to_range(
        case_id, secret=settings.ANONYMIZATION_SECRET_KEY, low=-MAX_DATE_SHIFT, high=MAX_DATE_SHIFT
    )
    return (
...
class AnonymizationMixin:
    ...
    @classmethod
    def _setup(cls, model_class: Type[BaseModel], config: Optional[AnonymizationConfig] = None):
        if config:
    ...
    def anonymize_data(self) -> Self:
        ...
        for field in self.__anonymization_fields__:
            ...
            anonymizer = self.__anonymization_functions__.get(field, self.anonymize_value)
