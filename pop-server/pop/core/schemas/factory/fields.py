...
def get_schema_field(
    ...
    if field.is_relation:
        resolver_fcn, python_type, serialization_alias, default = process_relation_field(field, exclude_related_fields, expand)
     
    else:
        python_type, serialization_alias, default, default_factory = process_non_relation_field(field)        
        if is_array_field:
            python_type = List[python_type]
        
    field_info = create_field_info(field, serialization_alias, default=default, default_factory=default_factory, optional=optional, expanded=expand)
...
def process_non_relation_field(field):
    ...
    return python_type, serialization_alias, default, default_factory
...
def process_relation_field(field, exclude_related_fields=None, expand=False):
    ...
    if expand:
        ...
        resolver_fcn, related_type, serialization_alias = get_related_field_type(field, related_model, serialization_alias)
    ...
    return resolver_fcn, python_type, serialization_alias, default
...
def get_related_field_type(field, related_model, serialization_alias):
    ...
    if issubclass(related_model, CodedConceptModel):
        return None, CodedConceptSchema, serialization_alias
