...
import onconova.core.serialization.filters as f
from onconova.tests.models import MockCodedConcept, MockModel, OptionsEnum
...
class TestDjangoFilters(TestCase):
    ...
    def setUpTestData(cls):
        ...
        grandparentA, _ = MockCodedConcept.objects.get_or_create(
            code="grandparent-A", display="grandparent A"
        )
        parentA, _ = MockCodedConcept.objects.get_or_create(
            code="parent-A", display="parent A", parent=grandparentA
        )
        conceptA, _ = MockCodedConcept.objects.get_or_create(
            code="code-A", display="concept A", parent=parentA
        )
        parentB, _ = MockCodedConcept.objects.get_or_create(
            code="parent-B", display="parent B"
        )
        conceptB, _ = MockCodedConcept.objects.get_or_create(
            code="code-B", display="concept B", parent=parentB
        )

        cls.conceptA1, _ = MockCodedConcept.objects.get_or_create(
            code="code-A-1", display="concept A1", parent=parentA
        )
        cls.conceptA2, _ = MockCodedConcept.objects.get_or_create(
            code="code-A-2", display="concept A2", parent=parentA
        )
        cls.conceptB1, _ = MockCodedConcept.objects.get_or_create(
            code="code-B-1", display="concept B1", parent=parentB
        )
        cls.conceptB2, _ = MockCodedConcept.objects.get_or_create(
            code="code-B-2", display="concept B2", parent=parentB
        )
    ...
    def assert_filtering(self, field, FilterClass, value, expected):
        ...
        self.assertTrue(
            filtered_queryset.count() > 0, "Filter resulted in an empty queryset."
        )
        self.assertTrue(
    ...
    @parameterized.expand(
        [
            ...
            (
                f.BetweenDatesFilter,
                [datetime(1999, 1, 1), datetime(2001, 1, 1)],
                "instanceA",
            ),
            (
                f.NotBetweenDatesFilter,
                [datetime(1999, 1, 1), datetime(2001, 1, 1)],
                "instanceB",
            ),
        ],
    ...
    @parameterized.expand(
        [
            (
                f.OverlapsPeriodFilter,
                (datetime(2000, 1, 1).date(), datetime(2009, 12, 12).date()),
                "instanceA",
            ),
            (
                f.NotOverlapsPeriodFilter,
                (datetime(2000, 1, 1).date(), datetime(2009, 12, 12).date()),
                "instanceB",
            ),
            (
                f.ContainsPeriodFilter,
                (datetime(2000, 1, 1).date(), datetime(2009, 12, 12).date()),
                "instanceA",
            ),
            (
                f.NotContainsPeriodFilter,
                (datetime(2000, 1, 1).date(), datetime(2009, 12, 12).date()),
                "instanceB",
            ),
        ],
