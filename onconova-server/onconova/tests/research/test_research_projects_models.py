...
from onconova.research.models.project import ProjectMembership
...
class ProjectDataManagerGrantModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.grant = factories.ProjectDataManagerGrantFactory.create(
            member=cls.authorized_user
        )
        cls.authorized_user.refresh_from_db()

        cls.revoked_user = factories.UserFactory.create(access_level=1)
        cls.grant = factories.ProjectDataManagerGrantFactory.create(
            member=cls.revoked_user, revoked=True
        )
