...
import { KaplanMeierCurve } from 'onconova-api-client';
...
@Component({
    ...
    selector: 'onconova-kappler-meier-curve',
    template: `
    <div class="chart-container" style="height: {{height()}}; width: {{width()}}">
        <canvas #KapplerMeierCurve></canvas>
        @if (chart) {
            <onconova-cohort-graph-context-menu [target]="KapplerMeierCurve" [chart]="chart" [data]="data()"/>
        }
    </div>`
})
